...
public class CarsRepo {
    ...
    public Cars addCar(Cars c) {
        ...
        String sql2 = "INSERT INTO documentation (car_status, car_registration, mileage, car_id) VALUES (?,?,?,@car_id)";
        template.update(sql2, c.getCar_status(), c.getCar_registration(), c.getMileage());

        return null;
    }

    public Cars findCarById(int documentation_id){
        String sql = "SELECT cars.car_id,brand,model,beds,documentation_id,car_status,car_registration,mileage FROM documentation " +
    ...
    public Boolean deleteCar(int documentation_id){
        ...
        String sql = "DELETE FROM documentation WHERE car_id = ?";
    ...
    public Cars updateCar(int car_id, Cars c) {
        ...
        String sql1 = "UPDATE documentation SET car_status = ?, car_registration = ?, mileage = ? WHERE documentation_id =  ?";
        template.update(sql1, c.getCar_status(), c.getCar_registration(), c.getMileage(), c.getCar_id());
        return null;
    }
}